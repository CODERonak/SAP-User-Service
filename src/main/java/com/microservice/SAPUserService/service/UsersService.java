...
import com.microservice.SAPUserService.exceptions.custom.DuplicateException;
import com.microservice.SAPUserService.exceptions.custom.NotFoundException;
...
public class UsersService {
        ...
        if (repository.existsByEmail(request.getEmail()))
            throw new DuplicateException(
                    "Email: " + request.getEmail()
                            + " has already been taken, if you want to create a new account please use a different email");
    ...
    public UserResponse getUserById(Long userId) {
        Users user = repository.findById(userId)
                .orElseThrow(() -> new NotFoundException("User not found with the id " + userId));
